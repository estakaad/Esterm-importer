...
def import_concepts(file, dataset, max_objects=5000000):
    ...
    saved_filename = f'files/import/esterm-22-11-23/{timestamp}_concepts_saved.json'
    not_saved_filename = f'files/import/esterm-22-11-23/{timestamp}_concepts_not_saved.json'
...
def update_word_ids(filename, dataset, concepts_dataset):
    ...
    timestamp = datetime.now().strftime('%Y-%m-%d_%H-%M-%S')
    words_without_id_file = f'files/import/esterm-22-11-23/{timestamp}_words_without_id2.json'
    words_with_more_than_one_id_file = f'files/import/esterm-22-11-23/{timestamp}_words_with_more_than_one_id2.json'


    with open(f'files/import/esterm-22-11-23/concepts_with_word_ids1.json', 'w', encoding='utf-8') as file:
